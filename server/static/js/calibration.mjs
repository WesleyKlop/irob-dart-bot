...
export const calculateAngle = (tableheight, x) => {
    const y_diff = (BOARD_HEIGHT + BULLS_EYE_HEIGHT) - (tableheight + CONSTRUCTION_HEIGHT)
    const ans_disc = (Math.pow(DART_SPEED, 4) - G * (G * Math.pow(x, 2) + 2 * y_diff * Math.pow(DART_SPEED,2)))
